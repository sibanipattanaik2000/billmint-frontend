...
export default function NewSalePage() {
  ...
  const updateItem = (
    ...
    setItems((prev) =>
      prev.map((item) => {
        ...
        // Auto calculate amount when price / qty / discount changes
        if (
          field === "price" ||
          field === "qty" ||
  ...
  const handleSave = () => {
    ...
    console.log("Saving Sale Invoice:", payload);
  ...
  return (
    <div className="flex-1 flex flex-col bg-white overflow-hidden">
      ...
      {/* Address + Salesman */}
      <div className="px-6 py-3 grid grid-cols-1 md:grid-cols-3 gap-6 border-b">
        <div>
        ...
        <div>
          <label className="text-xs text-gray-500">SALES MAN</label>
          <input
            type="text"
            value={salesman}
      ...
      <div className="border-t px-6 py-4 grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div>
          ...
          <div className="flex justify-between text-sm">
            <span>Balance</span>
            <span>{received ? 0 : totalAmount.toLocaleString("en-IN")}</span>
          </div>
        </div>
      </div>
